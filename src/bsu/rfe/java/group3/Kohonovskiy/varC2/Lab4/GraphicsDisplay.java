package bsu.rfe.java.group3.Kohonovskiy.varC2.Lab4;
...
public class GraphicsDisplay extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (setRotate) {
            canvas.scale(getSize().getWidth() / getSize().getHeight(), getSize().getHeight() / getSize().getWidth());
            canvas.rotate(-Math.PI / 2);
            canvas.translate(-getSize().getWidth(), 0);
        }
        if (showGrid)
            paintGrid(canvas);
        if (showAxis)
    ...
    public class MouseHandler extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent ev) {
            ...
            if (selectedMarker >= 0) {
                ...
                selectionRect.setFrame(ev.getX(), ev.getY(), 1, 1);
            }
        }
    ...
    public class MouseMotionHandler implements MouseMotionListener {
        ...
        public void mouseDragged(MouseEvent ev) {
            if (changeMode) {
                double[] currentPoint = pointToXY(ev.getX(), ev.getY());
